...
class Main
  ...
  def remove_station_on_route
    puts 'Введите название станции:'; name = gets.chomp.capitalize
    puts 'и номер маршрута:'; number = gets.chomp
    has_station?(name).length.zero?
    puts 'There is no such station.'
    station = stations.select { |item| item.name == name }
    has_route?(number).length.zero?
  ...
  def change_train_station
    ...
    loop do
      ...
      choice = gets.to_i
      break unless [1, 2].include?(choice)
  ...
  def add_carriage_to_train
    ...
    if train.type == 'passenger'
      puts 'Укажите колличество мест в вагоне:'
      train.type == 'cargo'
      puts 'Indicate the volume of the wagon:'
      quantity = gets.chomp
      ...
      puts 'Carriage added to the train'
    else
      puts 'Train not found'
    end
    end
  ...
  def show_stations_list_on_route
    ...
    route = has_route?(number).first
      ...
      puts "Stations on the route:"; route.stations.map { |station| puts station.name }
  end
    ...
    if Train.find(number)
      ...
      train.show_carriage_in_train do |item|
        ...
        if item.type == 'passenger'
          puts "Свободно мест:   #{item.free_places}, Занятых мест:       #{item.occupied_places}"
        end
